...
/// Allow only reads, injecting the `github-app` credential.
...
async fn allowed_read_injects_real_credential() {
    ...
    halter.register("agent-1", policy_from(READ_ONLY));
...
async fn pr_create_gated_by_base_branch() {
    ...
    let halter = start_halter(&upstream.base_url).await;
    halter.add_credential("github-app", "real-secret-token");
    halter.register("agent-1", policy_from(PR_TO_DEVELOP_ONLY));
    let token = halter.mint_token("agent-1", 3600).await;
...
/// User story: the admin API refuses to mint for an unregistered agent.
#[tokio::test]
async fn admin_mint_unknown_agent_is_not_found() {
    let upstream = start_mock_upstream().await;
    let halter = start_halter(&upstream.base_url).await;
    let resp = halter.mint("ghost", 3600).await;
